    ...
    <div className={`w-1/4 mb-4 relative shadow-sm rounded-lg bg-white border ${borderColor} ${hasFire ? undefined : 'opacity-75'}`}>
      ...
      <div className={`p-2 pb-3 text-center`}>
        <p className={`font-bold text-sm sm:text-lg`}>
          {onJury && (
            <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 fill-current inline-block mr-1 mb-1 text-orange-900" viewBox="0 0 1792 1792">
        ...
        {(advantages.length > 0) && (
          <p className="text-xs font-bold leading-tight">
            {(advantages.length > 0) && advantages.map(advantage => <>📜 {advantage}<br /></>)}
          </p>
        )}
      </div>
    </div>
